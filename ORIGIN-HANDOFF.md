...
**0. Peer-to-peer push notification verification with Bego (~15m).**
The send/receive UX was tested end-to-end on May 20 evening with the test account. All four recipient intents (Stack / Replace / Save / Decline) work correctly, the badge counts and clears, the Received section renders cards, the activate-from-archive picker works. **Push notification arrival was NOT verified** — Sofia attempted send-to-self via the test account but her phone didn't get a push (likely cause: per-device push subscriptions, the phone doesn't have one even though her laptop browser does). Next session: test with Bego, who already has notifications enabled on her own device. Send from Sofia's account → Bego's email → confirm she gets a Web Push titled `Sofia sent you a protocol · {name} · N supplements`. If it doesn't arrive, the console log in `dbNotifyProtocolSent` will show the edge function's response (look for `[push] notify_protocol_sent response 200 {...}` in devtools). Most likely failure modes: (a) `reason: 'no-subscriptions'` → recipient hasn't enabled notifications on the device they're holding, (b) `reason: 'vapid-not-configured'` → VAPID env vars missing on the Supabase project (they're set since daily reminders work, so unlikely), (c) HTTP 4xx → auth or payload bug.
