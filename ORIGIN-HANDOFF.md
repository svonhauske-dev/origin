...
**0. Protocol Templates surface — IN PROGRESS, only App.jsx wiring left (~1-2h).**
Original design (locked May 18 afternoon): Protocol Library and Templates are sibling clinician surfaces — distinct verbs (run vs share). Both surface protocols owned by the clinician; the `is_template` flag distinguishes them. Sidebar nav entry `"Templates"` below My Origin (compact row treatment, FileText icon). Surface heading `"Protocol Templates"` + `"+ New template"` button (top right) + list of template rows (name + supplement count + N-patients-sent stat). Each row clickable → opens ProtocolDetailScreen for editing. Per-row actions: **Send to patient** (Popover with patient picker, reuses `dbSendProtocol`) and **Use for myself** (clones template into a new owned protocol with `is_template=false` — taking your own template into rotation). Aside collapses on Templates surface (no patient context).

**Status as of May 20 mid-day:**
- ✅ **DB migration run** — `protocols.is_template` column exists (verified via REST probe). `protocol_sends` table also exists with `source_protocol_id` (created earlier in demo-setup.sql).
- ✅ **API helpers complete** in `src/lib/api.js` (uncommitted): `dbGetProtocols` filtered to `is_template=is.false`, new `dbGetTemplates`, `dbGetTemplateSendCounts(templateIds, t)` returning send rows for client-side grouping, `dbCloneTemplateForOwner(template, templateSupps, t)` that POSTs a new protocol + clones supps with `is_template=false` and `status='active'`.
- ✅ **`Templates.jsx` UI scaffolded** (uncommitted, new file): `TemplateRow` with name + supp count + sent-count caption + Send icon (opens Popover with patient picker) + Use-for-myself icon + chevron. Empty state with dashed border. Header with "+ New template" CTA.
- ✅ **Sidebar.jsx wired** (uncommitted): `Templates` button below My Origin with `FileText` icon and `activeNavItem === 'templates'` active state.
- 🟡 **App.jsx — imports + state declared, rest not done** (uncommitted): `Templates` component imported, `dbGetTemplates` / `dbGetTemplateSendCounts` / `dbCloneTemplateForOwner` imported, `templates` and `templateSendCounts` state added. **Still needed:** (a) load templates in the protocol-load effect (call `dbGetTemplates` after `dbGetProtocols`, then `dbGetTemplateSendCounts` if the user is a clinician with sent templates, group by `source_protocol_id` into a `{ templateId: count }` map), (b) routing branch for `activeNavItem === 'templates'` (render `<Templates ... />` in the main column, collapse the aside as roster view does), (c) implement 4 handlers — `onCreateTemplate` (opens new-protocol flow with `is_template: true` preset), `onOpenTemplate` (opens existing ProtocolDetailScreen, scoped to the template — note ProtocolDetailScreen may need a small prop tweak if it currently assumes non-template protocols), `onSendToPatient(template, patientId)` (calls existing `dbSendProtocol`), `onUseForMyself(template)` (load template supps → `dbCloneTemplateForOwner` → toast + refresh protocols list).
- ⏳ **Test + ship** — verify all 4 flows in dev, confirm aside collapses, confirm "+ New template" doesn't collide with existing new-protocol flow, commit + push to main. Pre-push checklist: at least one template created via "+ New template", one Send-to-patient round trip, one Use-for-myself clone that appears in ProtocolLibrary.
