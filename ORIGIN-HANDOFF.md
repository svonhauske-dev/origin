...
*Last updated: May 18, 2026 (late evening, ~midnight) — **Web Push refill diagnosis complete; cron job partially deployed but verification deferred.** The hard problem is solved at the code level (3 commits: 9b461d4, bb3392a, d78dd29). What's NOT yet confirmed working is the pg_cron job actually invoking the deployed function. Three sources of friction during attempted verification: (1) Supabase API gateway requires JWT on incoming function calls by default — addressed by `verify_jwt = false` in `supabase/config.toml`, but Supabase's GitHub auto-deploy may not honor the config flag (still being verified); (2) CRON_SECRET env var management took several attempts (current value: 64-char hex `7c8d3f91a4e25b86c0d72f1a5e94b380fcd6a7e2185b4f93c08d6e7a1f2c5b94` set via Dashboard → Edge Functions → Secrets); (3) deploy lag on auto-deploys from GitHub varies 2-10 min. **For tonight (May 18 / early May 19): Sofia and Bego will each open the app once to trigger JWT-mode recompute, which refills the queue for the next 48h.** When picking this back up: (a) confirm `d78dd29` actually deployed by checking Dashboard timestamp on `recompute_notifications`; (b) trigger the function with X-Cron-Secret and see if the diagnostic JSON (added in d78dd29 to the 401 response) returns `cron_secret_mismatch` with lengths — that tells us if the env var matches or has whitespace issues; (c) if Supabase's GitHub integration genuinely doesn't honor `verify_jwt = false` from config.toml, alternative approaches that sidestep this entirely: **Vercel Cron Job** (calls the function with anon key, no Supabase secret management needed) OR **piggyback on `process_notifications_queue`** (the every-minute drain that already runs cleanly — extend it to opportunistically call `recomputeForUser` for users whose queue depth is below a threshold). Both alternatives are simpler than the current pg_cron + custom secret approach.*
