...
export default function ManageSupplementsSheet({ open, onClose, supplements, token, onEdit, onDelete, onTogglePause, onResume }) {
  ...
  // Reset to active view when sheet closes
  useEffect(() => {
    if (!open) {
      setViewMode("active");
      setConfirmId(null);
  ...
  // Fetch adherence counts when stopped tab is shown
  useEffect(() => {
    if (!open || viewMode !== "stopped" || !token) return;
    const ids = stoppedSupps.map(s => s.id);
    if (!ids.length) return;
    dbGetAdherenceCounts(ids, token)
      .then(counts => setAdherenceCounts(counts))
      .catch(() => {});
  }, [open, viewMode, token, stoppedSupps.length]);
